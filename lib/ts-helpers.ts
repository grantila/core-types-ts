...
export function wrapAnnotations<T extends ts.Node>(
	...
	const comment = stringifyAnnotations(node, {
		formatWhitespace: blockComment,
	}).trim();

	if (!comment) return tsNode;
...
export function makeGenericComment(
	...
	const enwrap = (comment: string) => (bodyOnly ? comment : `/*${comment}*/`);
...
export type HeaderOptions = Pick<
	ToTsOptions,
	| 'filename'
	| 'sourceFilename'
	| 'userPackage'
...
export function createCodeHeader({
	...
	if (noDisableLintHeader && noDescriptiveHeader) return '';
