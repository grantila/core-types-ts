...
export function convertTypeScriptToCoreTypes(
	...
	): statement is TopLevelDeclaration =>
	...
	const recurseNamespaces = (
		...
		...statements
			.filter((statement): statement is ts.NamespaceDeclaration =>
		...
		...statements
			...
			.map((statement): ExportedDeclaration | undefined => {
				if (namespaceParents.length > 0 && namespaces === 'ignore')
					return undefined;
	...
	const renameDeclaration = ({
		declaration,
		namespaceParents,
	}: ExportedDeclaration): TopLevelDeclaration => {
		const name = declaration.name.getText();

		const fullName =
			namespaces === 'hoist' || namespaces === 'ignore'
				? name
				: [...namespaceParents, name].join(
	...
	const declarations = flattenAndFilterConflicts(
...
function handleGeneric(node: ts.Node, ctx: Context) {
	return ctx.handleError(
		ctx.getUnsupportedError(`Generic types are not supported`, node),
	);
}

function fromTsTopLevelNode(
...
function fromTsTypeNode(
	...
	if (ts.isUnionTypeNode(node))
		...
		const getStringUnion = (su: ts.TypeNode): undefined | string[] => {
			if (ts.isLiteralTypeNode(su) && ts.isStringLiteral(su.literal))
				return [su.literal.text];

			if (!ts.isUnionTypeNode(su))
				return ctx.handleError(
					ctx.getUnsupportedError(`Expected string union kind`, su),
				);
		...
		if (ref === 'Omit') {
			...
			const filteredMembers: typeof members = {
				...
				properties: Object.fromEntries(
					Object.entries(members.properties).filter(
						([name]) => !secondNames.includes(name),
					),
				),
			};
			...
			const filteredMembers: typeof members = {
				...members,
				properties: Object.fromEntries(
					Object.entries(members.properties).filter(([name]) =>
						secondNames.includes(name),
					),
				),
			};
			...
			const filteredMembers: typeof members = {
				...members,
				properties: Object.fromEntries(
					Object.entries(members.properties).map(([name, value]) => [
						name,
...
function getRefType(
	...
	} else if (typeInfo) return { type: 'ref', ref };
...
function fromTsTuple(
	...
	const elementTypes = elements.map(
		(node) => fromTsTypeNode(tsStripOptionalType(node), ctx) ?? anyType,
	);
	const additionalItems = rest
		? fromTsTypeNode((rest.type as ts.ArrayTypeNode).elementType, ctx) ??
		  anyType
		: false;
	const firstOptional = elements.findIndex((node) =>
